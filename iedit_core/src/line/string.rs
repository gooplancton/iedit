...
impl CharacterEditable for str {
    ...
    fn split_chars_at(&self, idx: usize) -> (&Self, &Self) {
        let char_idx = self.char_indices().nth(idx).map(|(i, _)| i).unwrap_or(self.len());
...
impl EditorLine for String {
    ...
    fn delete_chars(&mut self, range: impl std::ops::RangeBounds<usize>) {
        let start = match range.start_bound() {
            std::ops::Bound::Included(&idx) => self.char_indices().nth(idx).map(|(i, _)| i).unwrap_or(self.len()),
            std::ops::Bound::Excluded(&idx) => self.char_indices().nth(idx + 1).map(|(i, _)| i).unwrap_or(self.len()),
            std::ops::Bound::Unbounded => 0,
        };
        let end = match range.end_bound() {
            std::ops::Bound::Included(&idx) => self.char_indices().nth(idx + 1).map(|(i, _)| i).unwrap_or(self.len()),
            std::ops::Bound::Excluded(&idx) => self.char_indices().nth(idx).map(|(i, _)| i).unwrap_or(self.len()),
    ...
    fn get_chars(&self, range: std::ops::Range<usize>) -> &Self::SliceType {
        ...
        let end = self.char_indices().nth(range.end).map(|(i, ch)| i + ch.len_utf8() - 1).unwrap_or(self.len());
        &self[start..end]
    }
